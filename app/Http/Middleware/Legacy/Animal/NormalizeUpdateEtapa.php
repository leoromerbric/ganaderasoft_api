...
class NormalizeUpdateEtapa
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (!$isV2) {
            ...
            $legacyEtapaId = $request->route('etapa');

            if ($animalId && $legacyEtapaId) {
                // Buscamos el ID del registro único autoincremental en la tabla intermedia N:M
                $surrogateId = EtapaAnimal::where('animal_id', $animalId)
                    ->where('etapa_id', $legacyEtapaId)
                    ->value('id');
                ...
                // Reemplazamos el parámetro en la ruta dinámica de Laravel
                $request->route()->setParameter('etapa', $surrogateId);
            }
        }
