...
class NormalizeStoreCambiosAnimal
{
    /**
    ...
    {
        $animalEtapaId = null;
        if (isset($input['cambios_etapa_anid']) && isset($input['cambios_etapa_etid'])) {
            $animalEtapaId = DB::table('animal_etapa')
                ->where('animal_id', $input['cambios_etapa_anid'])
                ->where('etapa_id', $input['cambios_etapa_etid'])
                ->value('id');
        }

        return [
            'fecha_cambio'    => $input['Fecha_Cambio'] ?? null,
            'etapa_cambio'    => $input['Etapa_Cambio'] ?? null,
